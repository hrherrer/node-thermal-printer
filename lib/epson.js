...
module.exports = {
  ...
  printQR: function printQR(str, settings) {
    ...
    if (settings.model) {
      if (settings.model === 1) append(config.QRCODE_MODEL1);else if (settings.model === 3) append(config.QRCODE_MODEL3);else append(config.QRCODE_MODEL2);
    } else {
  ...
  pdf417: function pdf417(data, settings) {
    ...
    if (settings.truncated) append(config.PDF417_OPTION_TRUNCATED);else append(config.PDF417_OPTION_STANDARD);
  ...
  maxiCode: function maxiCode(data, settings) {
    ...
    if (settings.mode) {
      if (settings.mode == 2) append(config.MAXI_MODE2);else if (settings.mode == 3) append(config.MAXI_MODE3);else if (settings.mode == 5) append(config.MAXI_MODE5);else if (settings.mode == 6) append(config.MAXI_MODE6);else append(config.MAXI_MODE4);
    } else {
  ...
  printImageEpson: function printImageEpson(image, callback) {
    var png = new PNG({
      filterType: 4
    });
  ...
  _printImageBufferEpson: function _printImageBufferEpson(width, height, data, callback) {
