...
function append(buff) {
  if (buffer) buffer = Buffer.concat([buffer, buff]);else buffer = buff;
}
...
module.exports = {
  ...
  code128: function code128(data, settings) {
    ...
    if (settings) {
      if (settings.text == 1) append(config.BARCODE_CODE128_TEXT_1);else if (settings.text == 2) append(config.BARCODE_CODE128_TEXT_2);else if (settings.text == 3) append(config.BARCODE_CODE128_TEXT_3);else if (settings.text == 4) append(config.BARCODE_CODE128_TEXT_4);
    } else {
    ...
    // 33 - Large
    if (settings) {
      if (settings.width == "SMALL") append(config.BARCODE_CODE128_WIDTH_SMALL);else if (settings.width == "MEDIUM") append(config.BARCODE_CODE128_WIDTH_MEDIUM);else if (settings.width == "LARGE") append(config.BARCODE_CODE128_WIDTH_LARGE);
    } else {
      append(config.BARCODE_CODE128_WIDTH_LARGE);
    }

    // Barcode height
    if (settings && settings.height) append(new Buffer([settings.height]));else append(new Buffer([0x50]));
  ...
  // ----------------------------------------------------- PRINT IMAGE STAR -----------------------------------------------------
  printImageStar: function printImageStar(image, callback) {
  ...
  _printImageBufferStar: function _printImageBufferStar(width, height, data, callback) {
