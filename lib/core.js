...
module.exports = {
  ...
  partialCut: function partialCut() {
    append(config.CTL_VT);
    append(config.CTL_VT);
  ...
  setBuffer: function setBuffer(newBuffer) {
  ...
  printVerticalTab: function printVerticalTab() {
    append(config.CTL_VT);
  },

  bold: function bold(enabled) {
    if (enabled) append(config.TXT_BOLD_ON);else append(config.TXT_BOLD_OFF);
  },

  underline: function underline(enabled) {
    if (enabled) append(config.TXT_UNDERL_ON);else append(config.TXT_UNDERL_OFF);
  },

  underlineThick: function underlineThick(enabled) {
    if (enabled) append(config.TXT_UNDERL2_ON);else append(config.TXT_UNDERL_OFF);
  },

  upsideDown: function upsideDown(enabled) {
    if (enabled) append(config.UPSIDE_DOWN_ON);else append(config.UPSIDE_DOWN_OFF);
  },

  invert: function invert(enabled) {
    if (enabled) append(config.TXT_INVERT_ON);else append(config.TXT_INVERT_OFF);
  },
  ...
  setTypeFontA: function setTypeFontA() {
    append(config.TXT_FONT_A);
  },

  setTypeFontB: function setTypeFontB() {
    append(config.TXT_FONT_B);
  },

  setTextNormal: function setTextNormal() {
    append(config.TXT_NORMAL);
  },

  setTextDoubleHeight: function setTextDoubleHeight() {
    append(config.TXT_2HEIGHT);
  },

  setTextDoubleWidth: function setTextDoubleWidth() {
    append(config.TXT_2WIDTH);
  },

  setTextQuadArea: function setTextQuadArea() {
  ...
  drawLine: function drawLine() {
    ...
    for (var i = 0; i < printerConfig.width; i++) {
      if (printerConfig.lineChar) append(new Buffer(printerConfig.lineChar));else append(new Buffer([196]));
    }
    module.exports.newLine();
  },

  leftRight: function leftRight(left, right) {
  ...
  tableCustom: function tableCustom(data) {
    var cellWidth = printerConfig.width / data.length;
    var secondLine = [];
  ...
  isPrinterConnected: function isPrinterConnected(exists) {
    iface.isPrinterConnected(exists);
  },

  // ----------------------------------------------------- PRINT QR -----------------------------------------------------
  printQR: function printQR(str, settings) {
    if (printerConfig.type === printerTypes.STAR) {
  ...
  // ----------------------------------------------------- PRINT BARCODE -----------------------------------------------------
  printBarcode: function printBarcode(data, type, settings) {
  ...
  // ----------------------------------------------------- PRINT MAXICODE -----------------------------------------------------
  maxiCode: function maxiCode(data, settings) {
    if (printerConfig.type === printerTypes.EPSON) {
  ...
  // ----------------------------------------------------- PRINT CODE128 -----------------------------------------------------
  code128: function code128(data, settings) {
    if (printerConfig.type === printerTypes.STAR) {
  ...
  // ----------------------------------------------------- PRINT PDF417 -----------------------------------------------------
  pdf417: function pdf417(data, settings) {
    if (printerConfig.type === printerTypes.STAR) {
  ...
  // ----------------------------------------------------- PRINT IMAGE -----------------------------------------------------
  printImage: function printImage(image, callback) {
    try {
  ...
  // ----------------------------------------------------- PRINT IMAGE BUFFER -----------------------------------------------------
  printImageBuffer: function printImageBuffer(buffer, callback) {
    var png = new PNG({
      filterType: 4
    }).parse(buffer);
...
var setInternationalCharacterSet = function setInternationalCharacterSet(charSet) {
...
// ------------------------------ Append ------------------------------
var append = function append(buff) {
  if (typeof buff === "string") {
    ...
    for (var i = 0; i < buff.length; i++) {
      ...
      if (endBuff) endBuff = Buffer.concat([endBuff, tempBuff]);else endBuff = tempBuff;
    }
